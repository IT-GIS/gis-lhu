...
export default async function EnglishPublicBlogPostPage({
  params,
}: {
  params: Params;
}) {
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <section className="relative flex min-h-screen items-end overflow-hidden bg-slate-200 px-4 py-16 text-white md:px-8">
        ...
        <div className="relative z-10 mx-auto w-full max-w-5xl">
          <Link
            href="/en/informasi"
            className="mb-6 inline-flex rounded-full bg-white/15 px-4 py-2 text-sm font-semibold backdrop-blur transition hover:bg-white/25"
          >
            Back to Information
          </Link>
          ...
          <h1 className="mt-3 text-3xl font-bold leading-tight tracking-tight md:text-6xl">
            {article.title}
          </h1>
        </div>
      </section>
      ...
      <article className="mx-auto max-w-4xl px-4 py-10">
        <div className="space-y-7 rounded-[32px] border border-white/70 bg-white p-8 shadow-[0_30px_80px_rgba(15,23,42,0.08)] md:p-10">
          <div className="flex flex-wrap gap-3 text-sm font-semibold text-slate-500">
            ...
            <span>/</span>
            <span>GIS Laboratory</span>
          </div>

          <p className="rounded-3xl bg-slate-50 p-5 text-base leading-8 text-slate-600">
            {article.excerpt}
          </p>
