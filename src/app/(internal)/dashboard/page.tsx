    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
        {kpiCards.map((kpi) => {
          ...
          return (
            <Card key={kpi.label} className="group relative overflow-hidden p-5 transition-all duration-300 hover:-translate-y-1">
              <div className={`absolute -right-8 -top-8 h-24 w-24 rounded-full bg-gradient-to-br ${kpi.tone} opacity-15 transition group-hover:scale-110 group-hover:opacity-25`} />
              <div className="relative flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-bold text-[var(--color-muted-foreground)]">{kpi.label}</div>
                  <div className="mt-2 text-3xl font-extrabold tracking-tight text-[var(--color-gis-navy)] dark:text-white">
                    {kpi.value}
                  </div>
                </div>
                <div className={`rounded-2xl bg-gradient-to-br ${kpi.tone} p-2.5 text-white shadow-glow`}>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="rounded-[28px]">
          <div className="text-lg font-bold text-[var(--color-gis-navy)] dark:text-slate-100">Aktivitas Terbaru</div>
            ...
            {data.latestActivities.length === 0 ? (
              ...
              data.latestActivities.map((log) => (
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{log.action}</div>
        ...
        <Card className="rounded-[28px]">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <div className="text-lg font-bold text-[var(--color-gis-navy)] dark:text-slate-100">Dokumen Terbaru</div>
              ...
              data.latestDocuments.map((doc) => (
                ...
                >
                  ...
                  <div className="mt-5 flex justify-end border-t border-sky-100 pt-4 dark:border-slate-800/80">
                    <Button variant="ghost" asChild className="h-8 rounded-full bg-sky-50 px-4 text-xs font-bold text-[var(--color-gis-blue)] transition-colors hover:bg-[var(--color-gis-blue)] hover:text-white dark:bg-slate-800 dark:text-[var(--color-gis-cyan)]">
